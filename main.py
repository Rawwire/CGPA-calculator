...
with st.container():
    ...
    if selected == "CGPA":
        ...
        for k in range(1, n1 + 1):
            if k == 1:
                z = st.number_input(f"Enter the {k}st Semester GPA:", key=f"gpa_{k}" , step=1, min_value=1)
                temp.append(z)
            elif k == 2:
                z = st.number_input(f"Enter the {k}nd Semester GPA:", key=f"gpa_{k+10}", step=1, min_value=1)
                temp.append(z)
            elif k == 3:
                z = st.number_input(f"Enter the {k}rd Semester GPA:", key=f"gpa_{k+20}", step=1, min_value=1)
                temp.append(z)
            else:
                z = st.number_input(f"Enter the {k}th Semester GPA:", key=f"gpa_{k+30}", step=1, min_value=1)
                temp.append(z)
        cgpa= sum(temp)/n1
        if st.button("Calculate"):
            st.header(f"CPGA for Semester {n1} is {cgpa}")
